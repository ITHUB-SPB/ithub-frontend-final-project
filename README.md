...
- [x] доработать компонент `Cta` (1 балл)
...
- [] доработать компонент `Featured` (1 балл за стилизацию мобильной версии, 1 балл за подгрузку изображений, 1 балл за динамическое выставление стилей фона и текста по свойствам из объектов, 3 балла за адаптив под десктоп)

### `store/app/pages/index`

- [] внести разработанные компоненты на главную страницу и стилизовать в соответствии с мобильным макетом (1 балл)
- [] стилизовать в соответствии с десктопным макетом (2 балла)

### Функционал корзины

- [x] счетчики количества товаров на изменение количества и удаление (2 балла), с интеграцией с хранилищем;
- [] форма оформления заказа (2 балла);
- [] сохранение заказов на сервере (2 балла);

### Функционал фильтрации по характеристикам

- [] пофиксить подсчет значений по характеристикам в `server/api/characteristics.get.ts`, корректно группируя и сортируя, фильтруя отсутствующие значения (2 балла)

- [] динамически выводить фильтры на странице `products`
...
- [x] добавить серверные функции для проверки данных и регистрации (2 балла)
...
- [x] разработать компонент для `CartPrice`: на вход должны приходить цена одного товара и функция для обработки клика на удаление товара, кнопки + и - должны функционировать, изменяя количество товаров, при этом стоимость должна пересчитываться (2 балла)

- [] написать стори для `CartPrice` (2 балла) 
...
4. (при отсутствии) Создайте файл `apps/store/.env.local` и внесите в него ключ
